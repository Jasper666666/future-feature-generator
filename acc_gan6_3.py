...
parser.add_argument('--ckpt-path', default="standford10_3.ckpt", help="path of pth file", type=str)
...
parser.add_argument('--epochs', default=30, help="training epoch", type=int)
...
def train(prenet,net, trainloader, epochs=30,save_path = 'stanford10.ckpt'):
    if whether_continue:
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(tq):
            ...
            z_img = z_img.cuda()
            f_f = prenet(torch.cat([image, z_img], 1))
            for i in range(1, K):
                z = Variable(torch.randn((image.size(0), z_dim)))
                z_img = z.view(z.size(0), z.size(1), 1, 1).expand(z.size(0), z.size(1), image.size(2), image.size(3))
                z_img = z_img.cuda()
...
def test(prenet,prenet2,net, net2,testloader, load_path = 'stanford10.ckpt'):
...
if __name__ == '__main__':
    ...
    '''
    prenet.load_state_dict(torch.load(prenet_ckpt),strict=True)
    prenet=prenet.cuda()
    net = model(num_class=num_classes).cuda()
    prenet2=myAlexNet()
    prenet2.load_state_dict(torch.load('zxalexnet.ckpt'),strict=True)
    prenet2=prenet2.cuda()
    net2=model(num_class=num_classes)
    net2.load_state_dict(torch.load('standford10_1024_8313.ckpt'),strict=True)
    net2=net2.cuda()
    ...
    # print(net)
    if mode == "traintest":
        train(prenet,net, trainloader,epochs, ckpt_path)
        test(prenet,prenet2,net, net2,testloader, ckpt_path)
    elif mode == "train":
        train(prenet,net, trainloader, epochs,ckpt_path)
    else:
        test(prenet,prenet2,net, net2,testloader, ckpt_path)
